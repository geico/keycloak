...
=== Brute-force user status includes property counters

The response from `GET /admin/realms/\{realm}/attack-detection/brute-force/users/\{userId}` now includes a `properties` object containing the status of each protected user property. With the default lock policy, this object contains the existing per-user counter under the `id` property.

Clients that deserialize this response into a closed schema or compare the complete JSON object must accept the additional field. Existing top-level fields and the default per-user lock behavior are unchanged.
...
`BruteForceProtector.failedLogin` and `successfulLogin` gained compatible default methods that receive the attempted identifier. Custom brute-force providers that implement only the previous signatures continue to work.
