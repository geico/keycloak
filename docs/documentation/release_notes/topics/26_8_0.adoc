...
== Lock brute-force attacks by user properties

Brute-force detection can now maintain counters for selected built-in or custom user properties. Failed logins increment the per-user counter and/or only the submitted identifier, each with its own threshold. Reaching either threshold locks the account. The existing per-user behavior remains the default.

== Independent brute-force counters per authentication channel

Selected authentication channels, identified by the reference category of the authenticator such as `password` or `otp`, can now keep their own brute-force failure counter with its own threshold. Failures on one channel no longer consume the attempts of another. A realm chooses whether reaching a channel threshold locks the whole account or only blocks further attempts on that channel. Custom authenticators, including SMS and email OTP providers, get their own counter by declaring their own reference category. Sharing a single counter across channels remains the default.
